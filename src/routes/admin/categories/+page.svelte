<script lang="ts">
  ...
  // isBusy = keamanan (disable semua tombol, cegah race condition)
  // processingId = visual saja (overlay ditarget ke card yang sedang diproses)
  ...
  {:else}
    <div class="space-y-2">
      {#each data.categories as category, index (category.id)}
        <BusyOverlay active={processingId === category.id}>
